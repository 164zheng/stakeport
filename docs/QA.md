...
| Buyer queues junk EIP-7251 requests ahead of the fill, then claims "not accepted" before its request is dequeued | `proveNotAccepted` only takes a state `REQUEST_DEQUEUE_DELAY` (6 h) after the fill. The predeploy dequeues 2 requests per block and its fee grows as e^(excess/17) with queue ≤ excess, so a 1800-block delay would cost ~e^211 wei per request (`test_proveNotAccepted_revertsWhileRequestMayBeQueued`). |
| Nobody relays checkpoint 1, buyer takes both the stake and a `refundExpired` refund | `acceptWindow` is capped at `MAX_ACCEPT_WINDOW` (2 days): an accepted pair stays in `pending_consolidations` for ≥ 261 epochs (~27.8 h) and EIP-4788 keeps roots ~27.3 h, so acceptance is provable for ≥ ~55 h after the fill. Settlement is permissionless; the relayer, the seller or anyone can prove it within the window. |
| Missed slots skew epoch checks | The state root proof also binds the header slot (node 4 of the header tree = hash(slot, proposer_index)); fills and deliveries use the exact epoch of the proven state instead of one derived from the EIP-4788 timestamp. |
